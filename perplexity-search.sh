...
# Check the capslock state and press it again if required to make sure we're lowercase before popping the dialog
sleep 0.1
...
if [ -n "$query" ]; then
  ...
  xdotool key ctrl+t
  xdotool type "https://www.perplexity.ai/?q=$queryUrlEncoded"
  xdotool key Return
fi